  ...
  <div class="container h-100">
    <div class="row h-100 align-items-center">
      <div class="col-12 col-md-6 offset-md-3">
        ...
        <hr />
        <form @submit="reset" class="row" method="post">
          <input type="hidden" name="token" v-model="auth.token" />
          <div class="form-group col-12">
            <label for="inputEmail" class="font-weight-bold">Email</label>
            <input type="email" name="email" v-model="auth.email" id="inputEmail" placeholder="Insert Your Email" class="form-control" required />
...
export default {
  ...
  methods: {
    ...
    reset: function(event) {
      ...
      this.message = null;
      axios
        .post('/api/password/reset', this.auth)
        .then(response => {
          this.signIn();
        })
        .catch(({ response: { data } }) => {
